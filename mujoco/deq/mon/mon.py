...
class MONSingleFc(nn.Module):
    ...
    def init_inverse(self, alpha, beta):
        I = torch.eye(self.A.weight.shape[0], dtype=self.A.weight.dtype, device=self.A.weight.device)
...
def init_fft_conv(weight, hw):
    ...
    kernel = F.pad(F.pad(kernel, (0, hw[0] - weight.shape[2], 0, hw[1] - weight.shape[3])), (0, py, 0, px), mode="circular")[
        :, :, py:, px:
    ]
    return fft_to_complex_matrix(n2o_fft(kernel, 2, onesided=False))


def n2o_fft(x, ndim, onesided=False):
    if ndim != 2 or onesided:
        raise NotImplemented
    x_fft = nfft.fftn(torch.complex(x, torch.zeros_like(x)), dim=(-1, -2))
    x_fft = torch.view_as_real(x_fft)
    return x_fft


def n2o_ifft(x, ndim, onesided=False):
    if ndim != 2 or onesided:
        raise NotImplemented
    x_ifft = torch.view_as_complex(x)
    x_ifft = nfft.ifftn(x_ifft, dim=(-1, -2))
    x_ifft = torch.view_as_real(x_ifft)[..., 0]
    return x_ifft
...
def fft_conv(x, w_fft, transpose=False):
    ...
    """
    x_fft = n2o_fft(x, 2, onesided=False)
    x_fft = fft_to_complex_vector(x_fft)
    ...
    return n2o_ifft(wx_fft, 2, onesided=False)
...
class MONSingleConv(nn.Module):
    ...
    def init_inverse(self, alpha, beta):
        ...
        I = torch.eye(Afft.shape[1], dtype=Afft.dtype, device=Afft.device)[None, :, :]
...
class MONMultiConv(nn.Module):
    def __init__(self, in_channels, conv_channels, image_size, kernel_size=3, m=1.0):
        super().__init__()
        self.pad = 4 * ((kernel_size - 1) // 2,)
        self.conv_shp = tuple((image_size - 2 * self.pad[0]) // 2**i + 2 * self.pad[0] for i in range(len(conv_channels)))
        self.m = m
        ...
        self.A_n0 = nn.ModuleList(
            [nn.Conv2d(c1, c2, kernel_size, bias=False, stride=2) for c1, c2 in zip(conv_channels[:-1], conv_channels[1:])]
        )

        self.g = nn.ParameterList([nn.Parameter(torch.tensor(1.0)) for _ in range(len(conv_channels))])
        self.gn = nn.ParameterList([nn.Parameter(torch.tensor(1.0)) for _ in range(len(conv_channels) - 1)])
        self.h = nn.ParameterList([nn.Parameter(torch.tensor(1.0)) for _ in range(len(conv_channels))])
    ...
    def z_shape(self, n_batch):
        return tuple((n_batch, self.A0[i].in_channels, self.conv_shp[i], self.conv_shp[i]) for i in range(len(self.A0)))
    ...
    def bias(self, x):
        ...
        for i in range(n - 1):
            b_out.append(torch.zeros(z_shape[i + 1], dtype=self.A0[0].weight.dtype, device=self.A0[0].weight.device))
        return tuple(b_out)
    ...
    def multiply(self, *z):
        def multiply_zi(z1, A1, B1, A1_n=None, z0=None, A2_n=None):
            ...
            if A2_n is not None:
                ...
                A2_nTA2_nz1 = self.unstride(F.conv_transpose2d(A2_nz1, A2_n, stride=2))
                out -= A2_nTA2_nz1
            if A1_n is not None:
        ...
        for i in range(1, n - 1):
            z_out.append(multiply_zi(z[i], self.A(i), self.B(i), A1_n=self.A_n(i - 1), z0=z[i - 1], A2_n=self.A_n(i)))
        z_out.append(multiply_zi(z[n - 1], self.A(n - 1), self.B(n - 1), A1_n=self.A_n(n - 2), z0=z[n - 2]))
    ...
    def multiply_transpose(self, *g):
        def multiply_zi(z1, A1, B1, z2=None, A2_n=None, A2=None):
            ...
            if A2_n is not None:
                ...
                A2_nTA2z2 = self.unstride(F.conv_transpose2d(self.unzpad(A2z2), A2_n, stride=2))
    ...
    def init_inverse(self, alpha, beta):
        n = len(self.A0)
        conv_fft_A = [init_fft_conv(self.A(i), (self.conv_shp[i], self.conv_shp[i])) for i in range(n)]
        conv_fft_B = [init_fft_conv(self.B(i), (self.conv_shp[i], self.conv_shp[i])) for i in range(n)]

        conv_fft_A_n = [init_fft_conv(self.A_n(i - 1), (self.conv_shp[i - 1], self.conv_shp[i - 1])) for i in range(1, n)]

        I = [
            torch.eye(2 * self.A0[i].weight.shape[1], dtype=self.A0[i].weight.dtype, device=self.A0[i].weight.device)[
                None, :, :
            ]
            for i in range(n)
        ]
        ...
        D1 = [
            (alpha + beta - beta * self.m) * I[i]
            - beta * conv_fft_A[i].transpose(1, 2) @ conv_fft_A[i]
            + beta * conv_fft_B[i]
            - beta * conv_fft_B[i].transpose(1, 2)
            for i in range(n - 1)
        ]
    ...
    def apply_inverse_conv(self, z, i):
        ...
        z0_fft = fft_to_complex_vector(torch.view_as_real(torch.fft.fft(z, dim=2)))
    ...
    def inverse_transpose(self, *g):
        ...
        for i in range(n - 2, -1, -1):
            ...
            A2_NTA2x2 = self.unstride(F.conv_transpose2d(self.unzpad(A2x2), self.A_n(i), stride=2))
