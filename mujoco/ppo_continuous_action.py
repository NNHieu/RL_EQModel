...
from pp0.agents import BaselineAgent, MonAgent
...
def parse_args():
    ...
    parser.add_argument("--agent-name", type=str, default="baseline",
...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"{args.env_id}__{args.exp_name}__{args.agent_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    agent = get_agent(args)(envs).to(device)
        ...
        for step in range(0, args.num_steps):
            ...
            for item in info:
                if "episode" in item.keys():
                    ...
                    if args.agent_name == "mon":
                        monsp_stats = agent.actor_mean.core.mon.stats
