...
def train(
    ...
    if lr_mode == "1cycle":
        lr_schedule = lambda t: np.interp([t], [0, (epochs - 5) // 2, epochs - 5, epochs], [1e-3, max_lr, 1e-3, 1e-3])[0]
    elif lr_mode == "step":
    ...
    if change_mo:
        max_mo = 0.85
        momentum_schedule = lambda t: np.interp([t], [0, (epochs - 5) // 2, epochs - 5, epochs], [0.95, max_mo, 0.95, 0.95])[0]
    ...
    for epoch in range(1, 1 + epochs):
        ...
        for batch_idx, batch in enumerate(trainLoader):
            ...
            if batch_idx % print_freq == 0 and batch_idx > 0:
                ...
                err = 100.0 * incorrect.float() / float(len(data))
                partialEpoch = epoch + batch_idx / len(trainLoader) - 1
                print(
                    "Train Epoch: {:.2f} [{}/{} ({:.0f}%)]\tLoss: {:.4f}\tError: {:.2f}".format(
                        partialEpoch, nProcessed, nTrain, 100.0 * batch_idx / len(trainLoader), ce_loss.item(), err
                    )
                )
        ...
        with torch.no_grad():
            ...
            err = 100.0 * incorrect.float() / float(nTotal)
            print("\n\nTest set: Average loss: {:.4f}, Error: {}/{} ({:.2f}%)".format(test_loss, incorrect, nTotal, err))
...
def cifar_loaders(train_batch_size, test_batch_size=None, augment=True):
    ...
    if augment:
        transforms_list = [transforms.RandomHorizontalFlip(), transforms.RandomCrop(32, 4), transforms.ToTensor(), normalize]
    else:
        transforms_list = [transforms.ToTensor(), normalize]
    train_dset = dset.CIFAR10("data", train=True, download=True, transform=transforms.Compose(transforms_list))
    test_dset = dset.CIFAR10("data", train=False, transform=transforms.Compose([transforms.ToTensor(), normalize]))

    trainLoader = torch.utils.data.DataLoader(train_dset, batch_size=train_batch_size, shuffle=True, pin_memory=True)

    testLoader = torch.utils.data.DataLoader(test_dset, batch_size=test_batch_size, shuffle=False, pin_memory=True)
...
def svhn_loaders(train_batch_size, test_batch_size=None):
    ...
    test_loader = torch.utils.data.DataLoader(
        dset.SVHN(root="data", split="test", download=True, transform=transforms.Compose([transforms.ToTensor(), normalize])),
        batch_size=test_batch_size,
        shuffle=False,
    )
