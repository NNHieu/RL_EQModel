...
def _animate_decision_area(model, X_train, y_train, steps, giffps, write2gif=False, file="nn_decision"):
    # (W, loss, acc) in steps
    ...
    def animate(i):
        ...
        ax.set_xlabel("x1")
        ax.set_ylabel("x2")
        step_text = ax.text(0.05, 0.9, "", fontsize=10, ha="left", va="center", transform=ax.transAxes)
        value_text = ax.text(0.05, 0.8, "", fontsize=10, ha="left", va="center", transform=ax.transAxes)
...
def animate_contour(
    ...
    (optim_point,) = ax.plot(true_optim_point[0], true_optim_point[1], "bx", label="target local minimum")
    plt.legend(loc="upper right")

    step_text = ax.text(0.05, 0.9, "", fontsize=10, ha="left", va="center", transform=ax.transAxes)
    value_text = ax.text(0.05, 0.75, "", fontsize=10, ha="left", va="center", transform=ax.transAxes)
    ...
    global anim
    anim = FuncAnimation(fig, animate, frames=len(param_steps), interval=100, blit=False, repeat=False)
...
def sample_frames(steps, max_frames):
    ...
    if max_frames > steps_len:
        warnings.warn(f"Less than {max_frames} frames provided, producing {steps_len} frames.")
