...
class Dir2D(object):
    ...
    def to_tensor(self, **kwargs):
        self.tensors = (
            Direction.to_tensor(self._dir[0], **kwargs),
...
class Surface:
    ...
    def get_unplotted_indices(self, layer):
        ...
        # negative and those indexces will be selected again and calculated over and over.
    ...
    def run(self, evaluate, inds, coords, max_inds_nums):
        ...
        with torch.no_grad():
            ...
            for count, ind in enumerate(inds):
                ...
                self.logger.info(
                    "Evaluating rank %d  %d/%d  (%.1f%%)  coord=%s \t%s \ttime=%.2f \tsync=%.2f"
                    % (
                        self.rank,
                        count,
                        len(inds),
